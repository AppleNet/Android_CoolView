...
import com.llc.android_coolview.mcgrady.slidingmenu.SlidingMenu.OnClosedListener;
import com.llc.android_coolview.mcgrady.slidingmenu.SlidingMenu.OnOpenedListener;
...
public class CustomViewAbove extends ViewGroup {
	...
	void setCurrentItemInternal(int item, boolean smoothScroll, boolean always, int velocity) {
		...
		if (smoothScroll) {
			if(item == 0){
				...
				smoothSecondScrollTo(destX, 0, velocity);
			}
	...
	public int getSecondContentWidth() {
		if (mViewBehind == null) {
			return 0;
		} else {
			return mViewBehind.getSecondaryContentWidth();
		}
	}
	...
	void smoothSecondScrollTo(int x, int y, int velocity) {
		if (getChildCount() == 0) {
			// Nothing to do.
			setScrollingCacheEnabled(false);
			return;
		...
		final int width = getSecondContentWidth();
	...
	public boolean onTouchEvent(MotionEvent ev) {
		...
		case MotionEvent.ACTION_UP:
			if (mIsBeingDragged) {
				...
				if(scrollX >= 0){					
					pageOffset = (float) (scrollX - getDestScrollX(mCurItem)) / getSecondContentWidth();
				}else{
