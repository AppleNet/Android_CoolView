...
public class MyProgressDialog extends View {
	...
	public MyProgressDialog(Context context, AttributeSet attrs, int defStyle) {
	...
	protected void onDraw(Canvas canvas) {
		...
		int radius = (int) (center - roundWidth / 2 - 5);// 圆环半径
		...
		if (textIsDisplayable && percent != 0 && style == STROKE) {
			canvas.drawText(percent + "%", center - textWidth / 2,
		 ...
		 */
		// 设置进度是实心还是空心
		paint.setStrokeWidth(roundWidth + 5);// 设置圆环宽度
	...
	/**
	 * 设置进度，此为线程安全控件，由于考虑多线的问题，需要同步 刷新界面调用postInvalidate()能在非UI线程刷新 @param
	 * progress
	 */
